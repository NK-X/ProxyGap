...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="append",
        dest="condition_ids",
        help="Evaluate only the named configured condition; may be repeated.",
    )
...
def evaluate_episode(
    *,
    ...
    scene: Path,
    heights: np.ndarray,
    terrain_gradient_x: np.ndarray,
    terrain_gradient_y: np.ndarray,
    route: Polyline,
    ...
    try:
        ...
        maximum_tilt = 0.0
        maximum_speed_schedule_slope = 0.0
        scheduled_speed_sum = 0.0
        minimum_scheduled_speed = speed
        ...
        for step_index in range(horizon_steps):
            ...
            if route_mode == "waypoint_route":
                ...
                speed_schedule = condition.get("high_level_speed_schedule")
                if speed_schedule is not None:
                    current_gx = float(
                        terrain_values(
                            terrain_gradient_x,
                            float(position_before[0]),
                            float(position_before[1]),
                            half_extent,
                        )
                    )
                    current_gy = float(
                    ...
                    schedule_slope = math.degrees(
                        math.atan(
                            max(
                                math.hypot(current_gx, current_gy),
                                math.hypot(target_gx, target_gy),
                            )
                        )
                    )
                    low_angle = float(speed_schedule["full_speed_below_degrees"])
                    high_angle = float(speed_schedule["minimum_speed_at_degrees"])
                    full_speed = float(speed_schedule["full_speed_m_per_s"])
                    low_speed = float(speed_schedule["minimum_speed_m_per_s"])
                    if not 0.0 <= low_angle < high_angle:
                        raise ValueError("Invalid slope-speed angle schedule")
                    if not 0.0 < low_speed <= full_speed:
                        raise ValueError("Invalid slope-speed velocity schedule")
                    fraction = float(
                        np.clip(
                            (schedule_slope - low_angle) / (high_angle - low_angle),
                            0.0,
                            1.0,
                        )
                    )
                    scheduled_speed = full_speed + fraction * (low_speed - full_speed)
                    env.set_task_speed(scheduled_speed)
                    maximum_speed_schedule_slope = max(
                        maximum_speed_schedule_slope, schedule_slope
                    )
                    minimum_scheduled_speed = min(minimum_scheduled_speed, scheduled_speed)
                else:
                    schedule_slope = float("nan")
                    scheduled_speed = speed
                scheduled_speed_sum += scheduled_speed
                env.slow_radius = (
                ...
                traces.append(
                    {
                        ...
                        "speed_schedule_slope_degrees": schedule_slope,
                        "scheduled_cruise_speed_m_per_s": scheduled_speed,
        ...
        row = {
            ...
            "high_level_speed_schedule_enabled": int(
                condition.get("high_level_speed_schedule") is not None
            ),
            "mean_scheduled_cruise_speed_m_per_s": scheduled_speed_sum
            / max(completed_steps, 1),
            "minimum_scheduled_cruise_speed_m_per_s": minimum_scheduled_speed,
            "maximum_speed_schedule_slope_degrees": maximum_speed_schedule_slope,
...
def main() -> None:
    ...
    terrain_gradient_y, terrain_gradient_x = np.gradient(
        heights,
        2.0
    ...
    if args.phase != "route-only":
        ...
        if args.condition_ids:
            requested = list(dict.fromkeys(str(value) for value in args.condition_ids))
            available = {str(item["condition_id"]): item for item in conditions}
            missing = [identifier for identifier in requested if identifier not in available]
            if missing:
                raise ValueError(f"Unknown condition identifiers: {missing}")
            conditions = [available[identifier] for identifier in requested]
        seeds = (
        ...
        for condition in conditions:
            for seed in seeds:
                row, episode_trace, audit = evaluate_episode(
                    ...
                    scene=scenes[0],
                    heights=heights,
                    terrain_gradient_x=terrain_gradient_x,
                    terrain_gradient_y=terrain_gradient_y,
                    route=route,
                    condition=condition,
                    seed=seed,
                    settings=config,
    ...
    execution = {
        ...
        "evaluated_condition_ids": list(
            dict.fromkeys(str(row["condition_id"]) for row in rows)
        ),
