...
def validate_config(
    ...
    _required_equal(
        slip["primary_denominator"],
        "physics_substeps_with_at_least_one_distal_foot_in_contact_and_normal_force_at_least_1N",
        "corrected-slip primary denominator",
    )
    _required_equal(
        slip["secondary_denominator"],
        "physics_substeps_with_at_least_one_distal_foot_contact_of_any_force",
        "corrected-slip secondary denominator",
    )
    _required_equal(
        slip["zero_primary_denominator"],
        "non_evaluable",
        "corrected-slip zero-denominator rule",
    )
...
def compiled_contract_audit(
    ...
    *,
    ...
    return {
        ...
        "seed_status": "construction_only_pending_vecenv_seed",
...
def record_effective_reset_seeds(
    vec_env: VecMonitor,
    audits: list[dict[str, Any]],
    *,
    training_seed: int,
) -> list[dict[str, Any]]:
    """Set and record the seeds that the next vector reset will consume."""

    effective = [int(value) for value in vec_env.seed(int(training_seed))]
    expected = [int(training_seed) + index for index in range(len(audits))]
    if effective != expected:
        raise RuntimeError(
            f"Stable Baselines3 effective worker seeds changed: {effective} != {expected}"
        )
    corrected: list[dict[str, Any]] = []
    for rank, (record, effective_seed) in enumerate(zip(audits, effective)):
        row = dict(record)
        row["worker_rank"] = rank
        row["effective_first_reset_seed"] = effective_seed
        row["seed_status"] = "construction_and_effective_first_reset_recorded"
    ...
    *,
    ...
    while not (terminated or truncated):
        ...
        for substep_index, substep in enumerate(rows, start=1):
            ...
            force_qualified_support = bool(
                np.any(
                    contacts
                    & (forces >= float(slip["minimum_normal_force_n"]))
                )
            )
            force_qualified_support_rows.append(force_qualified_support)
                ...
                traces.append(
                    {
                        ...
                        "force_qualified_supported": int(force_qualified_support),
    ...
    force_qualified_supported = np.asarray(
        force_qualified_support_rows, dtype=bool
    )
    if force_qualified_supported.shape != supported.shape:
        raise RuntimeError("Force-qualified support shape changed")
    qualified_any = np.any(candidate, axis=1)
    sustained_any = np.any(sustained, axis=1)
    supported_count = int(np.sum(supported))
    force_qualified_supported_count = int(np.sum(force_qualified_supported))
    ...
    row = {
        ...
        "force_qualified_supported_physics_substep_count": force_qualified_supported_count,
...
def aggregate_episode_rows(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    force_qualified_supported = sum(
        int(row["force_qualified_supported_physics_substep_count"])
        for row in rows
    )
    qualified_slip = sum(
        int(row["qualified_slip_physics_substep_count"]) for row in rows
    )
    sustained_slip = sum(
        int(row["corrected_sustained_slip_physics_substep_count"])
        for row in rows
    )
    slip_events = sum(int(row["corrected_slip_event_count"]) for row in rows)
    force_qualified_slip_evaluable = force_qualified_supported > 0
    result = {
        ...
        # Compatibility fields retain the attempt-0 any-contact denominator.
        # They are secondary diagnostics for future runs, not the primary gate.
        "qualified_slip_per_supported_fraction": qualified_slip / max(1, supported),
        "corrected_sustained_slip_per_supported_fraction": sustained_slip / max(1, supported),
        "corrected_slip_event_count": slip_events,
        "supported_physics_substep_count": supported,
        "force_qualified_supported_physics_substep_count": force_qualified_supported,
        "force_qualified_slip_evaluable": force_qualified_slip_evaluable,
        "qualified_slip_per_force_qualified_supported_fraction": (
            qualified_slip / force_qualified_supported
            if force_qualified_slip_evaluable
            else None
        ),
        "corrected_sustained_slip_per_force_qualified_supported_fraction": (
            sustained_slip / force_qualified_supported
            if force_qualified_slip_evaluable
            else None
        ),
        "primary_slip_denominator": "force_qualified_supported_physics_substeps",
        "secondary_slip_denominator": "any_contact_supported_physics_substeps",
    ...
    result["corrected_slip_events_per_100_force_qualified_supported_substeps"] = (
        100.0 * slip_events / force_qualified_supported
        if force_qualified_slip_evaluable
        else None
    )
    return result
...
def checkpoint_stop_decision(
    ...
    *,
    ...
    early = config["checkpoint_early_stopping"]
    slip_evaluable = bool(aggregate["force_qualified_slip_evaluable"])
    sustained_slip = aggregate[
        "corrected_sustained_slip_per_force_qualified_supported_fraction"
    ]
    slip_event_rate = aggregate[
        "corrected_slip_events_per_100_force_qualified_supported_substeps"
    ]
    failures = {
        "nonfinite": aggregate["nonfinite_episode_count"] > 0,
        "force_qualified_slip_non_evaluable": not slip_evaluable,
        ...
        "corrected_sustained_slip": bool(
            slip_evaluable
            and float(sustained_slip)
            > float(early["maximum_corrected_sustained_slip_per_supported_fraction_each_condition"])
        ),
        "corrected_slip_event_rate": bool(
            slip_evaluable
            and float(slip_event_rate)
            > float(early["maximum_corrected_slip_events_per_100_supported_substeps_each_condition"])
        ),
    ...
    qualified_warning = bool(
        slip_evaluable
        and float(aggregate["qualified_slip_per_force_qualified_supported_fraction"])
...
def smoke_engineering_decision(
    aggregate: dict[str, Any],
    *,
    condition_id: str,
    checkpoint_additional_timesteps: int,
) -> dict[str, Any]:
    """Fail closed on non-finite smoke results without applying science gates."""

    nonfinite_count = int(aggregate["nonfinite_episode_count"])
    if nonfinite_count > 0:
        raise RuntimeError(
            f"Smoke evaluation produced {nonfinite_count} non-finite episode(s)"
        )
    return {
        "condition_id": condition_id,
        "checkpoint_additional_timesteps": checkpoint_additional_timesteps,
        ...
        "nonfinite_fail_closed_passed": True,
    }
...
def final_gate(
    ...
    if not all(
        bool(row.get("force_qualified_slip_evaluable", False))
        for row in (control, pair0)
    ):
        return {
            "evaluable": False,
            "passed": False,
            "reason": "Primary force-qualified slip denominator is zero",
            "fixed_map_entry_authorised": False,
            "candidate_promoted": False,
        }
    ...
    observed = {
        ...
        "corrected_sustained_slip_fraction_delta": pair0["corrected_sustained_slip_per_force_qualified_supported_fraction"] - control["corrected_sustained_slip_per_force_qualified_supported_fraction"],
        "corrected_slip_events_per_100_delta": pair0["corrected_slip_events_per_100_force_qualified_supported_substeps"] - control["corrected_slip_events_per_100_force_qualified_supported_substeps"],
        "secondary_any_contact_corrected_sustained_slip_fraction_delta": pair0["corrected_sustained_slip_per_supported_fraction"] - control["corrected_sustained_slip_per_supported_fraction"],
        "secondary_any_contact_corrected_slip_events_per_100_delta": pair0["corrected_slip_events_per_100_supported_substeps"] - control["corrected_slip_events_per_100_supported_substeps"],
...
def artifact_inventory(output_root: Path) -> list[dict[str, Any]]:
    """Hash every completed artifact except the manifest that embeds the list."""
    ...
    try:
        ...
        for condition_id in config["training"]["condition_run_order"]:
            ...
            try:
                ...
                worker_audits = record_effective_reset_seeds(
                    vec_env,
                    worker_audits,
                    training_seed=int(config["training"]["training_seed"]),
                )
                write_json(
                    condition_root / "worker_contract_audits.json", worker_audits
                )
        ...
        manifest = {
            ...
            "runtime_code": {
                ...
                "frozen_config_path": str((output_root / "frozen_config.json").resolve()),
                "frozen_config_sha256": sha256(output_root / "frozen_config.json"),
            },
            ...
            "artifact_hashes_excluding_manifest": artifact_inventory(output_root),
        }
